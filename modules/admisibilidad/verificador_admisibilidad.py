...
class VerificadorAdmisibilidad:
    ...
    def _verificar_hojas_blanco(self):
        ...
        # Verificar si el PDF tiene objetos/contenido en cada página
        for num_pagina in range(self.total_folios):
            try:
                pagina = self.documento_pdf.pages[num_pagina]
                
                # Método 1: Intentar extraer texto directamente del PDF
                texto_extraido = pagina.extract_text().strip()
                texto_limpio = ''.join(c for c in texto_extraido if c.isalnum())
                
                tiene_texto_extraible = len(texto_limpio) > 0
                
                # Método 2: Verificar si la página tiene imágenes o contenido gráfico
                tiene_contenido_visual = False
                if '/Resources' in pagina:
                    recursos = pagina['/Resources']
                    # Verificar si tiene XObject (imágenes, formas)
                    if '/XObject' in recursos:
                        tiene_contenido_visual = True
                    # Verificar si tiene fuentes (aunque no se pueda extraer el texto)
                    if '/Font' in recursos:
                        tiene_contenido_visual = True
                
                # Método 3: Verificar que la página tenga stream de contenido
                if '/Contents' in pagina and pagina['/Contents']:
                    # Si tiene contenido en el stream, probablemente no está en blanco
                    tiene_contenido_visual = True
                
                # Una página se considera en blanco solo si:
                # - No tiene texto extraíble Y
                # - No tiene contenido visual (imágenes, fuentes, gráficos)
                esta_en_blanco = not (tiene_texto_extraible or tiene_contenido_visual)
                
                if esta_en_blanco:
                    hojas_blanco.append(num_pagina + 1)
                    print(f"  Folio {num_pagina + 1}: EN BLANCO")
                elif tiene_texto_extraible:
                    print(f"  Folio {num_pagina + 1}: OK ({len(texto_limpio)} caracteres de texto)")
                else:
                    print(f"  Folio {num_pagina + 1}: OK (contiene imágenes/gráficos - PDF escaneado)")
                    
            except Exception as e:
                print(f"  Folio {num_pagina + 1}: Error al analizar - {e}")
                # En caso de error, no lo marcamos como blanco (mejor asumir que tiene contenido)
        
        estado = "APROBADO" if len(hojas_blanco) == 0 else "RECHAZADO"
        detalles = []
